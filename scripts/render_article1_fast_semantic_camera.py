#!/usr/bin/env python3
"""Render clean full semantic-camera videos, previews and shared-route comparison."""
...
def main() -> int:
    ...
    ap.add_argument("--domain", choices=["car", "motorcycle", "both"], default="both")
    ap.add_argument("--skip-shared", action="store_true")
    ap.add_argument("--shared-only", action="store_true",
                    help="render only the spatially matched paired-route video")
    args = ap.parse_args()
    ...
    if not args.shared_only and args.domain in ("car", "both"):
        info["car"] = render_domain(
            args.car, out / "car_semantic_camera_full.mp4", cfg,
            out / "previews" / "car_preview_30s.mp4")
    if not args.shared_only and args.domain in ("motorcycle", "both"):
        info["motorcycle"] = render_domain(
            args.motorcycle, out / "motorcycle_semantic_camera_full.mp4", cfg,
            out / "previews" / "motorcycle_preview_30s.mp4")
    if (args.shared_only or args.domain == "both") and not args.skip_shared:
        info["shared_route"] = render_shared_route(
            args.car, args.motorcycle, out / "paired_shared_route_comparison.mp4", cfg)
